...
import type { BookingLine, BookingLineErrors, ContactErrors, ContactValues } from '../model/types';

import { getMaxBookingDateValue, getSelectedDateTime, getTodayDateValue } from './booking-date';
...
export function validateBookingLines(
    ...
    lines.forEach((line) => {
        ...
        const isPackage = selectedBookableItem?.kind === 'package';

        if (!line.bookableItemId) {
            lineErrors.bookableItemId = 'Выберите технику или пакет';
        }
        ...
        if (!line.bookingOptionId) {
            lineErrors.bookingOptionId = 'Выберите опцию';
        } else {
            const selectedOption = bookingOptions.find(
                (option) => option.id === line.bookingOptionId && option.isActive,
            );

            const expectedServiceId = selectedOption?.serviceId ?? '';

            if (
                !selectedOption ||
                selectedOption.bookableItemId !== line.bookableItemId ||
                expectedServiceId !== line.serviceId
            ) {
                lineErrors.bookingOptionId = 'Выбранная опция недоступна';
            }
        }
