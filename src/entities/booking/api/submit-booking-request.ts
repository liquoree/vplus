...
): NonNullable<BookingSubmitResult['code']> {
    if (backendCode === 'BOOKING_CONFLICT') {
        return 'BOOKING_CONFLICT';
    }

    if (backendCode === 'BOOKING_OUTSIDE_SEASON') {
        return 'BOOKING_OUTSIDE_SEASON';
    }

    if (backendCode === 'CAPTCHA_FAILED') {
        return 'CAPTCHA_FAILED';
    }

    if (backendCode === 'CAPTCHA_UNAVAILABLE') {
        return 'CAPTCHA_UNAVAILABLE';
    }

    if (backendCode === 'VALIDATION_ERROR' || status === 422) {
...
function getFirstFieldError(fields: Record<string, string> | null | undefined) {
    ...
    const message = Object.values(fields).find(
...
export async function submitBookingRequest(
    ...
    if (
        ...
        return {
            ...
            message: 'Данные заявки заполнены не полностью',
        };
    }

    const request = prepareBookingRequest(payload, captchaToken);

    const idempotencyKey = crypto.randomUUID();

    try {
        const response = await apiClient.post<BookingCreateResponse>('/bookings', request, {
            headers: {
        ...
        return {
            ...
            publicNumber: response.data.publicNumber,

            totalPrice: response.data.totalPrice,

            prepaymentPrice: response.data.prepaymentPrice,
        ...
        return {
            ...
            code: getSubmitErrorCode(backendError?.code, status),
