...
export async function getBookingAvailability({
    ...
    if (!bookableItemId || !date || !Number.isFinite(durationMinutes) || durationMinutes <= 0) {
    ...
    const response = await apiClient.get<BookingAvailabilityResult>('/bookings/availability', {
        params: {
            bookableItemId,
            date,
            durationMinutes,
