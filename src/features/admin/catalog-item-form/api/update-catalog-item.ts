...
function buildUpdateRequest(payload: CatalogItemFormSubmitPayload, version: number) {
    ...
    if (nextFileIndex !== imageFiles.length) {
...
export async function updateCatalogItem(
    ...
    const { request, files } = buildUpdateRequest(payload, version);
    ...
    try {
        const response = await apiClient.put<CatalogAdminMutationResponse>(
        ...
        if (!axios.isAxiosError<ApiErrorResponse>(error)) {
            throw new CatalogMutationError('Не удалось сохранить изменения.', 0);
        }
