...
import type { AdminLoginCredentials, AdminLoginResponse } from '../model/types';
...
export async function loginAdmin(credentials: AdminLoginCredentials): Promise<AdminLoginResponse> {
    try {
        ...
        if (!axios.isAxiosError<ApiErrorResponse>(error)) {
            throw new AdminLoginError('Не удалось выполнить вход', 0);
        }
        ...
        const message =
            backendMessage ??
            (status === 401 ? 'Неверный логин или пароль' : 'Не удалось выполнить вход');

        throw new AdminLoginError(message, status);
    }
}
