...
export default async function Page({ searchParams }: BookingRouteProps) {
    const params = await searchParams;

    const [itemsResult, bookingOptionsResult] = await Promise.allSettled([
        getCatalogItems(),
        getCatalogBookingOptions(),
    ]);

    if (itemsResult.status === 'rejected' || bookingOptionsResult.status === 'rejected') {
        return (
            <ErrorPage
                code="503"
    ...
    const bookingOptions = bookingOptionsResult.value;
